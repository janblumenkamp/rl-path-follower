...
keyboard.add_hotkey(',', keyboard_command, args=('yaw', -0.01))
keyboard.add_hotkey('.', keyboard_command, args=('yaw', 0.01))
...
def remote_control():
    ...
    while True:
        ...
        #drone.set_thrust(current_control['thrust']/5)
        drone.set_thrust(pif_h.step(desired_height, drone.position[2]))
        drone.step()
...
def eval_pid_speed_hor():
    ...
    while True:
        setpoint = 0
        if start_logging:
            setpoint = -1*angle_sign
        setpoints.append(setpoint)
        real_data.append(drone.trans_speed[1])
        timestep.append(current_time)
        current_time += 1/240
...
def eval_pid_speed_ver():
    ...
    while True:
        ...
        if start_logging:
            setpoint = 1*angle_sign
        setpoints.append(setpoint)
        real_data.append(-drone.trans_speed[2])
        timestep.append(current_time)
        current_time += 1/240
